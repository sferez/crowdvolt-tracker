...
/* All three are the same 32px circle -- two neutral, one in the device's
   colour. Grouped rather than repeated: styling #bell alone left the chat
   bubble a rounded square from .ghost, with its unread count unstyled. */
#bell, #chatBtn { position:relative; width:32px; height:32px; border-radius:50%; }
/* the count is already a red dot; a red border as well competes with the
   banner's outline for the same attention */
#bell.lit, #chatBtn.lit {
  color:var(--crit);
  background:color-mix(in srgb, var(--crit) 12%, var(--surface));
}
#bell .dot, #chatBtn .dot {
...
@media (max-width:820px) {
  ...
  /* three 36px buttons, two 6px gaps and the 16px margin: 136. Sized for two
     it was 98, and the stats line ran under the cluster. */
  header { position:relative; padding-right:140px; }
  .tools { position:absolute; top:10px; right:16px; margin:0; gap:6px; }
  #bell, #chatBtn, #profile { width:36px; height:36px; }
  .spacer { display:none; }
  #theme { order:1; }
  .stats { order:2; }
...
let TS_USED = false;      // this token has been spent; reset before reusing
let chatTimer = null;

const modToken = () => { try { return localStorage.getItem(MOD_KEY) || ''; }
                         catch (e) { return ''; } };

/* Moderation is a console affair on purpose. A password box in the settings
   panel told every visitor there was an admin door and invited them to knock;
   the token lives in this browser's storage instead, put there by hand:

     cvMod('the-token')   store it, and say whether the server agrees
     cvMod()              what is stored now, and whether it still works
     cvMod(null)          forget it

   The delete controls appear in chat once a token is stored. Whether it is
   the right one is settled by the server, never here. */
window.cvMod = async t => {
  try {
    if (t === null) { localStorage.removeItem(MOD_KEY); return 'forgotten'; }
    if (t) localStorage.setItem(MOD_KEY, String(t).trim());
  } catch (e) { return 'this browser will not store it'; }
  const v = modToken();
  if (!v) return 'nothing stored';
  try {
    const r = await fetch('/api/chat?op=auth',
                          {headers: {'authorization': 'Bearer ' + v}});
    if (r.ok && $('#chat').open) renderChat();
    return r.ok ? 'verified — delete controls are on'
                : 'the server does not recognise that token';
...
function chatToken() {
  ...
  // Reset here rather than after the last send. Tokens are single-use so one
  // is needed, but resetting on the way out leaves the widget sitting in the
  // composer asking to be verified for a message that already went.
  if (TS_USED) { window.turnstile.reset(TS_ID); TS_USED = false; }
...
async function chatSend(thread, root) {
  ...
  try {
    ...
    TS_USED = true;     // spent -- the next send resets before asking again
  }
}
