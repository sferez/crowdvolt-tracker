...
@media (max-width:1440px) {
  #list table, #deals table { display:block; overflow-x:auto; white-space:nowrap;
...
const dealTitle = (c, under) => {
  const v = Math.abs(c.vs_primary);
  return `${v < 1 ? exact(v) : money(v)} (${pctOf(under)}) below ${
    c.primary_basis === 'last-sold'
      ? `the last face value — ${money(c.primary)} before the primary sold out`
      : `fair value${c.primary != null ? ` — ${money(c.primary)} to buy direct` : ''}`}`;
};
...
function cardShell(e, id) {
  ...
  const spread = c.spread ?? (c.floor != null && c.bid ? c.floor - c.bid : null);
  const vs = vsSale(e);
  ...
  const pills = [
    ...
    c.bid ? `best bid ${money(c.bid)}` : null,
...
const COLUMNS = [
  ['date',    'Date',    'l', e => e.starts_ts ?? Infinity, cellDate],
  ['in',      'In',      'l', e => e.starts_ts ?? Infinity, cellIn],
  ['name',    'Event',   'l', e => (e.name || e.slug).toLowerCase(), cellName],
  ['sale',    'Last sale', '', e => e.current?.last_sale ?? -1,
    e => `<td>${e.current?.last_sale == null ? '—' : money(e.current.last_sale)}</td>`],
  // nothing to compare sorts to the end rather than reading as a big discount
  ['vsale',   'vs sale', '', e => vsSale(e) ?? Infinity, cellVsSale],
  ...
  ['prim',    'Fair value', '', e => primaryOf(e) ?? Infinity, e => cellFair(e, false)],
  ['vsprim',  'vs fair', '', e => vsPrimary(e) ?? Infinity, cellVsFair],
  ['venue',   'Venue',   'l', e => (e.venue || '').toLowerCase(),
    e => `<td class="l vn" title="${esc(e.venue || '')}">${esc(e.venue || '')}</td>`],
  ['city',    'City',    'l', e => (e.city || '').toLowerCase(),
    e => plainTd('l', esc(e.city || ''))],
  ['genre',   'Genre',   'l', e => genreLabel(e).toLowerCase(),
    e => plainTd('l', `<i class="gdot" style="background:${
      genreColor(genreOf(e))}"></i>${esc(genreLabel(e))}`)],
...
const DEAL_COLUMNS = COLUMNS.flatMap(c =>
  c[0] === 'prim' ? [[...c.slice(0, 4), e => cellFair(e, true)]] :
  c[0] === 'vsprim' ? [c, ['vsfpct', 'vs fair %', '',
                           e => vsPrimaryPct(e) ?? Infinity, cellVsFairPct]] : [c]);
...
function tableFor(cols, rows, sort) {
  const body = rows.map(e => `<tr data-slug="${esc(e.slug)}">${
    cols.map(c => c[4](e)).join('')}<td class="l">${e.status === 'active' ? ''
      : `<span class="badge">${esc(e.status)}</span>`}</td></tr>`).join('');
  return `<table><thead><tr>${cols.map(([k, t, cls]) =>
    `<th data-sort="${k}" class="${cls}"${sort.key === k
